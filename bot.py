...
DEV_GUILD_ID = 1466549361432461436  # force dev sync for now
    ...
    async with bot.pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO donations (
                server_id, user_id, item,
                quantity, donation_date, is_adjustment
            )
            VALUES ($1, $2, $3, $4, $5, $6)
...
async def inventory(
    ...
    for row in rows:
        lines.append(f"{row['item']}: {row['total'] or 0}")
...
async def lottery(
    ...
    try:
        ...
        await interaction.response.send_message(
            "You already entered or that number is taken.",
            ephemeral=True
        )
        return

    await interaction.response.send_message(
        f"{interaction.user.mention} entered with {number} 🎟️"
    )


@bot.tree.command(name="lottery_draw")
...
async def lottery_draw(interaction: discord.Interaction):
    ...
    async with bot.pool.acquire() as conn:
        ...
        winner = next(
            (row["user_id"] for row in rows if row["number"] == winning_number),
            None
        )
